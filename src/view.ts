...
export class TimeCanvasDashboardView extends ItemView {
    ...
    private expandedNotePaths: Set<string> = new Set();
    ...
    async renderDashboard(forceRerender = false) {
        ...
        try {
            ...
            this.expandedNotePaths = new Set();
            ...
            this.contentEl.querySelectorAll('.tc-note-card').forEach(card => {
                ...
                if (cw && link && path) {
                    if (cw.hasClass('tc-visible')) {
                        this.expandedNotePaths.add(path);
                    } else {
                        this.collapsedNotePaths.add(path);
                    }
                }
            });
            this.contentEl.querySelectorAll('.hist-year-header').forEach(header => {
                const yearStr = header.getAttribute('data-year');
            ...
            const btnToggleAll = btnGroup.createDiv({
                ...
                attr: {
                    title: this.allExpanded ? t('collapse_all') : t('expand_all'),
                    'aria-label': this.allExpanded ? t('collapse_all') : t('expand_all')
                }
            });
            const btnViewMode = btnGroup.createDiv({
                cls: 'tc-icon-btn tc-view-mode',
                attr: {
                    title: this.plugin.settings.viewMode === 'outline' ? t('toggle_snippet') : t('toggle_outline'),
                    'aria-label': this.plugin.settings.viewMode === 'outline' ? t('toggle_snippet') : t('toggle_outline')
                }
            });
            ...
            if (historyMap.size > 0) {
                ...
                for (const year of sortedYears) {
                    ...
                    let isExpanded = true;
                    if (this.plugin.settings.collapseYears && !this.allExpanded) {
                        isExpanded = this.expandedYears.has(year);
                    } else {
                        isExpanded = !this.collapsedYears.has(year);
                    }
                    ...
                    yearHeader.setAttribute('data-year', year.toString());
                    ...
                    for (const file of files) {
                        ...
                        if (!this.allExpanded) {
                            isNoteExpanded = this.expandedNotePaths.has(file.path);
                        } else {
                            isNoteExpanded = !this.collapsedNotePaths.has(file.path);
                        }
                        ...
                        setIcon(toggleIcon, isNoteExpanded ? 'chevron-down' : 'chevron-right');
                        ...
                        const cwClass = isNoteExpanded ? 'tc-content-wrapper tc-visible' : 'tc-content-wrapper';
                        const contentWrapper = noteCard.createDiv({ cls: cwClass });
    ...
    initIconsAndEvents(wrapper: HTMLElement, btnAll: HTMLElement, btnViewMode: HTMLElement) {
        ...
        this.registerDomEvent(btnViewMode, 'click', () => {
            ...
            btnViewMode.setAttribute('title', this.plugin.settings.viewMode === 'outline' ? t('toggle_snippet') : t('toggle_outline'));
            btnViewMode.setAttribute('aria-label', this.plugin.settings.viewMode === 'outline' ? t('toggle_snippet') : t('toggle_outline'));
        ...
        this.registerDomEvent(btnAll, 'click', () => {
            ...
            btnAll.setAttribute('title', this.allExpanded ? t('collapse_all') : t('expand_all'));
            btnAll.setAttribute('aria-label', this.allExpanded ? t('collapse_all') : t('expand_all'));

            if (this.allExpanded) {
                this.collapsedNotePaths.clear();
                this.collapsedYears.clear();
            } else {
                this.expandedNotePaths.clear();
            ...
            if (this.plugin.settings.collapseYears) {
                wrapper.querySelectorAll('.hist-year-header').forEach(header => {
                    ...
                    if (this.allExpanded) {
                        ...
                        if (yearNum !== null) {
                            this.collapsedYears.delete(yearNum);
                        }
                    } else {
                        body.setCssProps({ '--tc-max-height': `${body.scrollHeight}px` });
                        ...
                        if (yearNum !== null) {
                            this.collapsedYears.add(yearNum);
                        }
                    }
                });
            }
        ...
        this.registerDomEvent(wrapper, 'click', (e) => {
            ...
            if (header) {
                ...
                if (isExp) {
                    ...
                    if (yearNum !== null) {
                        this.collapsedYears.add(yearNum);
                        this.expandedYears.delete(yearNum);
                    }
                } else {
                    body.setCssProps({ '--tc-max-height': `${body.scrollHeight}px` });
                    body.addEventListener('transitionend', () => {
                    ...
                    if (iconBox) setIcon(iconBox as HTMLElement, 'chevron-down');
                    if (yearNum !== null) {
                        this.expandedYears.add(yearNum);
                        this.collapsedYears.delete(yearNum);
                    }
                }
                return;
            ...
            if (noteToggle) {
                ...
                if (contentWrapper) {
                    ...
                    if (isVisible) {
                        ...
                        if (path) {
                            this.collapsedNotePaths.add(path);
                            this.expandedNotePaths.delete(path);
                        }
                    } else {
                        ...
                        if (path) {
                            this.expandedNotePaths.add(path);
                            this.collapsedNotePaths.delete(path);
                        }
                    }
                }
                return;
